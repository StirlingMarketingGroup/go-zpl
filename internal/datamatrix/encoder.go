// Code derived from github.com/boombuler/barcode/datamatrix (MIT License).
...
// EncodeWithSize returns a Datamatrix barcode forced to the given symbol size
// (full symbol columns/rows including finder patterns, e.g. 22, 22).
func EncodeWithSize(content string, columns, rows int) (barcode.Barcode, error) {
	return EncodeWithSizeAndColor(content, columns, rows, barcode.ColorScheme16)
}

// EncodeWithSizeAndColor is like EncodeWithSize but uses the given color scheme.
func EncodeWithSizeAndColor(content string, columns, rows int, color barcode.ColorScheme) (barcode.Barcode, error) {
	data := encodeText(content)
	...
	if size == nil {
		return nil, fmt.Errorf("unsupported DataMatrix size %dx%d", columns, rows)
	}
	if len(data) > size.DataCodewords() {
		return nil, errors.New("to much data to encode")
	}
...
func addPadding(data []byte, toCount int) []byte {
	...
	for len(data) < toCount {
		...
		data = append(data, byte(tmp&0xFF))
	}
	return data
}
